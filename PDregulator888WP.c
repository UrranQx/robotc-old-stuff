...
{float u,v,es,eold,e,k1=5.4,k2=9.6;
	displayBigTextLine(2,"%d",SensorValue[S1]);
	displayBigTextLine(6,"%d",SensorValue[S2]);
	sleep(1);
	...
	while(true)
	{
		...
		if(SensorValue[S1]>=green1 && SensorValue[S1]<white1)
		{
			nMotorEncoder[motorB] = 0;
			motor[motorB] = -50;
			motor[motorC] = 50;
		...
		if(SensorValue[S2]>=green2 && SensorValue[S2]<white2)
		{	nMotorEncoder[motorB] = 0;
			motor[motorB] = 50;
			motor[motorC] = -50;
			while(nMotorEncoder[motorB]<155);//right//
